...
Format your response clearly with headings for each section. Be concise and professional. Use markdown formatting.`;
    ...
    try {
        ...
        try {
            ...
            return "🚨 **Proxy Server Not Running**\n\nThe backend server is not running. Please:\n\n1. **Open a new terminal**\n2. **Run:** `npm run server`\n3. **Or run both servers:** `npm run dev:full`\n\nThe server should start on http://localhost:3001\n\n**Why do we need this?** The OpenAI API cannot be called directly from the browser due to CORS restrictions.";
        }

        // Call the proxy server
        const response = await fetch(`${PROXY_SERVER_URL}/api/openai`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            if (response.status === 429) {
                return "⚠️ **API Quota Exceeded**\n\nYou've reached your API usage limit. Please check your OpenAI account for quota details or wait before trying again.";
            }
